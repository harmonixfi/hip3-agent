#!/usr/bin/env python3
"""Pull Ethereal market data and insert into arbit.db."""
...
def insert_instruments(conn, instruments: list) -> int:
    ...
    for inst in instruments:
        try:
            ...
            canonical_symbol = normalize_symbol("ethereal", raw_symbol)
            inst_id = inst["inst_id"]  # Keep the UUID as inst_id
            ...
            cursor.execute(
                ...
                (
                    ...
                    inst_id,            # Keep venue-specific inst_id (UUID)
                    "PERP",
                    inst["tickSize"],
                    inst["contractSize"],
                    inst["quote"],
...
def insert_funding(conn, funding_data: dict, ts_ms: int) -> int:
    ...
    Note: Ethereal's fundingRate1h is the hourly rate. We store it as-is.
    """
    cursor = conn.cursor()
    count = 0
    for symbol, funding_info in funding_data.items():
        try:
            # Normalize symbol to canonical form (base asset only)
            canonical_symbol = normalize_symbol("ethereal", symbol)

            rate = funding_info.get("fundingRate", 0.0)
            interval_hours = funding_info.get("fundingIntervalHours", 1)
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                ("ethereal", canonical_symbol, rate, interval_hours, None, ts_ms),
            )
            count += 1
        except Exception as e:
        ...
        try:
            ...
            if mark > 0:
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    ("ethereal", canonical_symbol, mark, index, last, None, None, (mark + index) / 2.0 if mark > 0 and index > 0 else mark, ts_ms),
                )
                count += 1
        except Exception as e:
        ...
        try:
            ...
            if mid > 0:
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    ("ethereal", canonical_symbol, mid, None, None, bid, ask, mid, ts_ms),
                )
                conn.commit()
                count += 1
...
def main() -> int:
    print("=== Ethereal Market Data Pull ===")
