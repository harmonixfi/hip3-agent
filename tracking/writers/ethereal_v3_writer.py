"""Ethereal -> DB v3 writer (append-only, idempotent).
...
Note: Ethereal instruments are perps only.
"""
...
def upsert_instruments(con: sqlite3.Connection, inst_rows: List[Dict]) -> int:
    """Insert or update Ethereal perp instruments."""
    sql = """
    ...
    vals = []
    for r in inst_rows:
        # Ethereal symbols are like "BTC-USD-PERP"
        raw_symbol = r["symbol"]
        ...
        specs = {
            "minPriceTickSize": r.get("minPriceTickSize", 0.1),
            "minQty": r.get("minQty", 1.0),
        }
...
def insert_prices(con: sqlite3.Connection, rows: List[Dict]) -> int:
    """Insert price data from bestBid/bestAsk."""
    sql = """
    INSERT OR IGNORE INTO prices_v3(
      venue, inst_id, ts, bid, ask, mid, last, source, quality_flags
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    vals = []
    for r in rows:
        vals.append(
            (
                ...
                r.get("mid"),
                r.get("last"),
                r.get("source", "ethereal:market_price"),
    ...
    for r in rows:
        vals.append(
            (
                ...
                r.get("source", "ethereal:funding_rate"),
                json.dumps(r.get("quality_flags", {}), separators=(",", ":")),
            )
        )
    con.executemany(sql, vals)
